...
from dataset.dataset_builder import build_dataset_test
...
def parse_args():
    ...
    parser.add_argument('--test_mode', type=str, default="val",
                        help="test mode should be val or train in test.py")
...
def test(args, test_loader, model):
    ...
    for i, (input, label, size, name) in enumerate(test_loader):
        ...
        # change output from probability into class
...
def test_model(args):
    ...
    # if save predict results create folder
    if args.save:
        if not os.path.exists(args.save_seg_dir):
            os.makedirs(args.save_seg_dir)

    # load the test set with label
    _, testLoader = build_dataset_test(args.dataset, args.num_workers, mode=args.test_mode)

    # Get the test result from the checkpoint.
    if not args.best:
        if args.checkpoint:
    ...
    # Get the best test result among the last 10 model(from current checkpoint) records.
    else:
        if args.checkpoint:
            if os.path.isfile(args.checkpoint):
...
if __name__ == '__main__':
    ...
    args = parse_args()
    args.test_mode = "train" # test mode val or train
