...
def build_dataset_train(dataset, input_size, batch_size, random_scale, random_mirror, num_workers):
    ...
    if not os.path.isfile(inform_data_file):
        ...
        if dataset == "TAS500":
            dataCollect = TAS500(data_dir, 19, inform_data_file=inform_data_file)
        else:
    ...
    trainLoader = data.DataLoader(
            TAS500(data_dir, crop_size=input_size, scale=random_scale,
                              mirror=random_mirror, mean=datas['mean']),
            batch_size=batch_size, shuffle=True, num_workers=num_workers,
            pin_memory=True, drop_last=True)

    valLoader = data.DataLoader(
            TAS500(data_dir, f_scale=1, mean=datas['mean']),
