...
class TAS500(Dataset):
    """
       ...
       Args:
        ...
        ignore_label: 255
    """
    def __init__(self, root, split='train', max_iters=None,crop_size=(512, 1024), mean=(), std=(), scale=True, mirror=True, ignore_label=255):
        ...
        for root_dir, dirs, files in os.walk(osp.join(root, 'tas500v1.1', split)):
            for file in files:
                img_file = osp.join(root_dir, file)
                label_file = osp.join(root_dir.replace(split,split+'_labels'),file)
                label_ids_file = osp.join(root_dir.replace(split+'_labels',split+'_labels_ids'),file)
    ...
    def __getitem__(self, index):
        ...
        id = cv2.imread(datafiles["ids"], cv2.IMREAD_GRAYSCALE)
        size = image.shape
        name = datafiles["name"]
...
class TASTestDataSet(Dataset):
    ...
    def __init__(self, root, mean=(128, 128, 128),
        ...
        self.mean = mean
        self.files = []
        for root_dir, dirs, files in os.walk(osp.join(root, 'tas500v1.1', 'test')):
            for file in files:
                img_file = osp.join(root_dir, file)
...
class TASTrainInform:
    ...
    def __init__(self, data_dir='', classes=23,
