...
# ==========================================================================================================================
# give gpu id if run with gpu
# ==========================================================================================================================
    if args.cuda:
        print("=====> use gpu id: '{}'".format(args.gpus))
        os.environ["CUDA_VISIBLE_DEVICES"] = args.gpus
        if not torch.cuda.is_available(): # check if gpu could be assecced
...
# ==========================================================================================================================  
    datas, trainLoader, valLoader = build_dataset_train(args.dataset, input_size, args.batch_size,
                                                        args.random_scale, args.random_mirror, args.num_workers)
    
    # number of batches ( one batch one iteration)
...
# ========================================================================================================================== 
    if args.resume:
        if os.path.isfile(args.resume):
            checkpoint = torch.load(args.resume) # resume: file name which saves checkpoint
    ...
    # set optimizer and store in args
    if args.optim == 'sgd':
        optimizer = optim.SGD(
...
def val(args, val_loader, model):
    ...
    for i, (input, label, size, name) in enumerate(val_loader):
        start_time = time.time()
        with torch.no_grad():
            # transfer input in cuda
            input_var = input.cuda()
            # predict one batch
            output = model(input_var)
        time_taken = time.time() - start_time
        print("[%d/%d]  time: %.2f" % (i + 1, total_batches, time_taken))
        # copy data from gpu to cpu and compute using numpy
        output = output.cpu().data[0].numpy()
        gt = np.asarray(label[0].numpy(), dtype=np.uint8) # data[0], label[0] : batch size is 1
        output = output.transpose(1, 2, 0) # channel last (K-classes)
        output = np.asarray(np.argmax(output, axis=2), dtype=np.uint8) # prediction is a one D array, containing K classes indices
